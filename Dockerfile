...
# Add script to install custom nodes (DEBUG: print + trace exact failure)
COPY scripts/comfy-node-install.sh /usr/local/bin/comfy-node-install
RUN set -eux; \
    echo "=== DEBUG: comfy-node-install path ==="; \
    ls -la /usr/local/bin/comfy-node-install; \
    echo "=== DEBUG: file type ==="; \
    file /usr/local/bin/comfy-node-install; \
    echo "=== DEBUG: show script (first 260 lines) ==="; \
    sed -n '1,260p' /usr/local/bin/comfy-node-install; \
    chmod +x /usr/local/bin/comfy-node-install; \
    echo "=== DEBUG: run with bash -x (full trace) ==="; \
    /bin/bash -x /usr/local/bin/comfy-node-install
...
RUN set -eux; \
    chmod +x /usr/local/bin/comfy-manager-set-mode
...
RUN set -eux; \
    mkdir -p models/checkpoints models/vae models/unet models/clip models/text_encoders models/diffusion_models models/model_patches
...
RUN set -eux; \
    if [ "$MODEL_TYPE" = "sdxl" ]; then \
      ...
      wget -q -O models/vae/sdxl-vae-fp16-fix.safetensors https://huggingface.co/madebyollin/sdxl-vae-fp16-fix/resolve/main/sd_xl_base_1.0.safetensors; \
    fi
...
RUN set -eux; \
    if [ "$MODEL_TYPE" = "flux1-dev-fp8" ]; then \
      wget -q -O models/checkpoints/flux1-dev-fp8.safetensors https://huggingface.co/Comfy-Org/flux1-dev/resolve/main/flux1-dev-fp8.safetensors; \
    fi

RUN set -eux; \
    if [ "$MODEL_TYPE" = "z-image-turbo" ]; then \
